  ...
  Hydrodynamics<ndim>* hydro)
{
  ...
  //-----------------------------------------------------------------------------------------------
#pragma omp parallel for default(shared) private(dn,dt,i,k,nstep) 
  ...
  Hydrodynamics<ndim>* hydro)
{
  ...
  //-----------------------------------------------------------------------------------------------
#pragma omp parallel for default(shared) private(dn,i,k,nstep)  
  for (i=0; i<sph->Nhydro; i++) {
...
#pragma omp parallel for default(shared) shared(sphdata, sph)
  ...
  Hydrodynamics<ndim>* hydro)
{
  ...
  ParticleType<ndim>* sphdata = reinterpret_cast<ParticleType<ndim>*>(sph->GetSphParticleArray());
  //-----------------------------------------------------------------------------------------------
#pragma omp parallel for default(shared) private(dn,i,level_new,nnewstep) reduction(+:activecount)
  for (i=0; i<sph->Nhydro; i++) {
    SphParticle<ndim>& part = sphdata[i];
    if (part.flags.is_dead()) continue;
