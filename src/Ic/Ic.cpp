  ...
  FLOAT *r)                              ///< [inout] Positions of particles
{
  if (ndim > 1) {
    ...
    // Loop over all required particles
    //---------------------------------------------------------------------------------------------
#pragma omp parallel for default(shared) private(i,j,k,phi,phiprime,phi1,phi2,rpos,Rmag,Rsqd)
    for (i=0; i<Npart; i++) {
  ...
  FLOAT *r)                            ///< [inout] Positions of particles
{
  ...
  // Loop over all required particles
  //-----------------------------------------------------------------------------------------------
#pragma omp parallel for default(shared) private(diff,i,xnew,xold)
  for (i=0; i<Npart; i++) {
    xnew = r[ndim*i];
  ...
  FLOAT *v)                            ///< [out] Velocities of particles
{
  ...
  if (ndim > 1) {
    ...
    // Loop over all required particles
    //---------------------------------------------------------------------------------------------
#pragma omp parallel for default(shared) private(dr,i,k,Rmag,Rsqd)
    for (i=0; i<Npart; i++) {
      for (k=0; k<ndim; k++) dr[k] = r[ndim*i + k] - rcentre[k];
  ...
  FLOAT *v)                              ///< [out] Interpolated particle velocity
{
  ...
  if (ndim == 3) {
    ...
    //---------------------------------------------------------------------------------------------
#pragma omp parallel for default(shared) private(dx,i,j,k,kk,p,vint) 
    for (p=0; p<Npart; p++) {
