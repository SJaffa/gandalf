...
//
//  Copyright (C) 2013  D. A. Hubber, G. Rosotti Edited by S Jaffa
//
...
void TurbulentCoreIc<ndim>::Generate(void)
{
  ...
  if (ndim == 3) {
    ...
    if (ic == "turbcore" || ic == "turbcore-uniform") {
    ...
    if (ic == "turbcore-radgrad") {
      ...
      FLOAT radgrad    = simparams->floatparams["radgrad"];
		  ...
		  // Create the sphere depending on the choice of initial particle distribution
		  if (particle_dist == "random") {
		    Ic<ndim>::AddralphaSphere(Npart, r, rcentre, radius, radgrad, sim->randnumb);
		  }
		  else {
